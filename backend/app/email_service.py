...
def send_donation_receipt_email(
    *,
    ...
    """Send an 80G donation receipt email.

    Returns True when the email was accepted by the SMTP server. Always
    returns False (instead of raising) when SMTP is not configured or the
    send fails, logging the reason.
    """
    if not is_smtp_configured():
        logger.warning(
            "SMTP is not configured; donation receipt NOT emailed to %s",
            to_email,
        )
        return False
    ...
    if not host or not _from_address():
        logger.warning(
            "SMTP host/from not configured; donation receipt NOT emailed to %s",
            to_email,
        )
        return False
    ...
    try:
        message = EmailMessage()
        message["Subject"] = "Your Donation Receipt - Piplad Welfare Foundation"
        message["From"] = formataddr(_from_address())
        message["To"] = to_email
        ...
        text_body = (
            ...
            "Foundation. Your donation receipt is included in this email. "
            "Kindly keep it for your records; an official 80G tax certificate "
        ...
        logger.info("Donation receipt emailed to %s", to_email)
        return True
    except Exception as exc:
        logger.error(
            "Failed to email donation receipt to %s: %s",
            to_email,
            exc,
        )
        return False