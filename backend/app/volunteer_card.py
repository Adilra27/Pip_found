"""Render the Piplad volunteer welcome card as a JPEG image.

The card is drawn with Pillow so the QR code and the volunteer's profile
photo are baked directly into the pixels. Unlike HTML attachments (whose
``data:`` and ``cid:`` image references Gmail/Outlook strip), a rendered
JPEG always displays the QR and photo in every e-mail client.
"""
...
# Palette (mirrors the HTML welcome-card design)
SLATE = (15, 23, 42)
...
def _load_pair(size: int) -> tuple[ImageFont.FreeTypeFont, ImageFont.FreeTypeFont]:
    """Return (bold, regular) fonts; bundled TTFs are preferred so desktop and
    Render output match exactly."""
    key = ("pair", size)
...
def _photo_layer(
    ...
    """Return a circular photo or initials avatar sized PHOTO_DIAM sq (RGBA)."""
...
def build_volunteer_card_jpg(
    *,
    ...
    """Render the volunteer welcome card and return JPEG bytes.

    Every input is drawn as pixels, so the card displays identically in all
    e-mail clients. Missing photo falls back to initials; missing QR simply
    omits the QR block. Raises only on unexpected failures (PIL, fonts).
    """
    ...
    photo_layer = _photo_layer(photo_bytes, photo_mime, full_name)
    image.paste(
        photo_layer,
    ...
    # Heading
    name = _clean(full_name) or "Volunteer"
    _draw_centered(
        draw,
    ...
    _draw_centered(
        draw,
        "We are thrilled to formally welcome you to the Piplad Welfare Foundation volunteer family.",
        CARD_W // 2,
        348,
        15,
        SUBTLE,
    ...
    rows = [
        ("Volunteer Name", name),
        ("Volunteer ID", _clean(volunteer_id) or "PWF-VOL-STANDBY"),
        ("Area of Interest", _clean(interest_area) or "General"),
        ("Phone", _clean(phone)),
    ...
    for idx, (label, value) in enumerate(rows):
        ...
        value_color = GREEN if label == "Volunteer ID" else INK
    ...
    if qr_png:
        ...
        _draw_centered(
            draw,
            f"Scan to verify: {_clean(volunteer_id) or 'PWF-VOL-STANDBY'}",
            QR_CENTER_X,
            QR_TOP + QR_SIZE + 18,
            16,
            INK,
        ...
        note_top = QR_TOP + QR_SIZE + 52
    else:
        note_top = box_bottom + 28

    # Note paragraph
    note_text = (
        "Your coordinator will reach out shortly with details of orientation, "
        "upcoming volunteer sessions, and how you can begin making a "
        "difference. Please keep your Volunteer ID handy for future events "
        "and communication."
    )
    regular = _load_pair(14)[1]
    note_lines = _wrap(draw, note_text, regular, box_right - box_left)
    for i, line in enumerate(note_lines):
        draw.text((box_left, note_top + i * 22), line, font=regular, fill=SUBTLE)

    # Footer band
    draw.rectangle((0, FOOTER_TOP, CARD_W, CARD_H), fill=SLATE)
    footer_regular = _load_pair(13)[1]
    footer_text = "Thank you for choosing to serve your community."
